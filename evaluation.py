...
from collaborative_filtering import (
    get_cf_recommendations_surprise,
)  # Removed get_als_recommendations
...
def precision_recall_at_k(predictions, true_relevant_items, k=config.N_RECOMMENDATIONS):
    ...
    precision = (
        num_relevant_in_predictions / len(predictions_at_k)
        if len(predictions_at_k) > 0
        else 0.0
    )
    recall = (
        num_relevant_in_predictions / len(true_relevant_items_set)
        if len(true_relevant_items_set) > 0
        else 0.0
    )
...
def evaluate_model(
    model_components,
    test_data,
    train_data,  # Pass train_data for filtering history
    user_col="user_id",
    item_col="item_id",
    k=config.N_RECOMMENDATIONS,
    model_type="svd",
):
    """
    ...
    train_user_items_history = (
        train_data.groupby(user_col)[item_col].apply(list).to_dict()
    )
    ...
    for user_id in test_users:
        ...
        try:
            if model_type == "svd":
                ...
                recommendations = get_cf_recommendations_surprise(
                    algo, user_id, trainset, item_map_eval, top_n=k
                )
            elif model_type == "cb":
                ...
                if user_liked:
                    ...
                    recommendations = get_content_based_recommendations(
                        user_liked, cosine_sim, item_indices, all_items_set, top_n=k
                    )
        ...
        # --- Removed ALS case ---
            ...
            print(
                f"Error getting recommendations for user {user_id} ({model_type}): {e}"
            )
