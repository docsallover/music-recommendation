...
def get_item_metadata_df(df_interactions, item_col='item_id', artist_col='artist', album_col='album', track_col='track'):
    ...
    if not all(col in df_interactions.columns for col in metadata_cols):
         print(f"Warning: Missing some metadata columns ({metadata_cols}). Proceeding with available ones.")
         metadata_cols = [col for col in metadata_cols if col in df_interactions.columns]


    df_metadata = df_interactions[metadata_cols].drop_duplicates(subset=[item_col]).reset_index(drop=True)
    print(f"Extracted unique item metadata for {df_metadata.shape[0]} items.")
    return df_metadata


def build_content_similarity_matrix(df_item_metadata, item_col='item_id', feature_cols=['artist', 'album', 'track']):
    """
    ...
    valid_feature_cols = [col for col in feature_cols if col in df_item_metadata.columns]
    if not valid_feature_cols:
         raise ValueError("No valid feature columns found in metadata for similarity calculation.")
    ...
    df_item_metadata['combined_features'] = df_item_metadata.apply(combine_features, axis=1)
...
def get_content_based_recommendations(user_liked_items, cosine_sim, item_indices, all_item_ids, top_n=config.N_RECOMMENDATIONS):
    """
    ...
    try:
        liked_indices = item_indices[valid_liked_items].tolist()
         ...
         print(f"Warning: Some liked items not found in item_indices: {e}. Skipping them.")
         valid_liked_items = [item for item in valid_liked_items if item in item_indices]
         if not valid_liked_items: return []
         liked_indices = item_indices[valid_liked_items].tolist()
...
if __name__ == '__main__':
    ...
    from preprocessing import create_item_id, filter_sparse_data # Need preprocessing steps

    df_inter = load_lastfm_data()
    if df_inter is not None:
        df_with_id = create_item_id(df_inter)
        df_filtered = filter_sparse_data(df_with_id) # Filter before extracting metadata
        ...
        if not df_meta.empty:
             ...
             try:
                 similarity_matrix, item_indices_map = build_content_similarity_matrix(df_meta, feature_cols=feature_list)

                 # Example usage:
                 # Get a user and their liked items from the filtered data
                 example_user = df_filtered['user_id'].iloc[0]
                 user_history = df_filtered[df_filtered['user_id'] == example_user]['item_id'].unique().tolist()

                 all_items_in_system = set(df_filtered['item_id']) # All items after filtering
