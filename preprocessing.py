...
def create_item_id(
    df, artist_col="artist", track_col="track", sep=config.ITEM_ID_SEPARATOR
):
...
def filter_sparse_data(
    ...
    print(
        f"Filtering sparse data (min user: {min_user_interactions}, min item: {min_item_interactions})..."
    )
    ...
    while True:
        ...
        df_filtered_items = df_filtered_users[
            df_filtered_users[item_col].isin(valid_items)
        ]
    ...
    final_valid_users = final_user_counts[
        final_user_counts >= min_user_interactions
    ].index
...
def create_user_item_matrix(df, user_col="user_id", item_col="item_id"):
    ...
    item_info = (
        df[["item_idx", "artist", "track"]].drop_duplicates().set_index("item_idx")
    )
    ...
    if num_users <= 0 or num_items <= 0:
        print(
            f"ERROR: Invalid matrix dimensions calculated: ({num_users}, {num_items})"
        )
        empty_matrix = csr_matrix((0, 0))
        empty_map = pd.DataFrame()
        return empty_matrix, empty_map, empty_map, df
    ...
    sparse_matrix = csr_matrix(
        (interaction_values, (df["user_idx"], df["item_idx"])),
        shape=(num_users, num_items),
    )  # Use calculated shape
...
def time_based_split(df, test_size=config.TEST_SET_SIZE, time_col="timestamp"):
    ...
    if time_col not in df.columns:
        raise ValueError(
            f"Timestamp column '{time_col}' required for time-based split."
        )
...
if __name__ == "__main__":
    ...
    if df_interactions is not None:
        ...
        if train_data is not None and not train_data.empty:
            ...
            sparse_matrix, user_map, item_map, train_data_indexed = (
                create_user_item_matrix(train_data.copy())
            )  # Use copy
            print("\nUser Map Head:")
            print(user_map.head())
            print("\nItem Map Head:")
