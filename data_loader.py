...
def load_lastfm_data(file_path=config.LASTFM_DATA_PATH):
    ...
    try:
        ...
        if not all(col in df.columns for col in expected_cols):
            raise ValueError(
                f"Data missing expected columns. Found: {df.columns.tolist()}"
            )
        ...
        if len(df) < original_rows:
            print(
                f"Dropped {original_rows - len(df)} rows with missing user_id, artist, or track."
            )
        ...
        try:
            df["timestamp"] = pd.to_datetime(
                df["date_str"] + " " + df["time_str"], errors="coerce"
            )  # errors='coerce' will turn unparseable dates into NaT
        except Exception as e:
            print(
                f"Could not combine Date and Time automatically: {e}. Trying specific format..."
            )
        ...
        if len(df) < original_rows:
            print(
                f"Dropped {original_rows - len(df)} rows due to missing or unparseable date/time."
            )
