...
def train_svd_model_implicit(df_interactions, user_col="user_id", item_col="item_id"):
    ...
    print(
        f"Surprise trainset built with {trainset.n_users} users and {trainset.n_items} items."
    )

    # Initialize and train the SVD algorithm
    # TODO: Tune parameters if needed
    algo = SVD(
        n_factors=config.N_LATENT_FACTORS_SVD,
        n_epochs=20,
        lr_all=0.005,
        reg_all=0.02,
        random_state=42,
...
def get_cf_recommendations_surprise(
    algo, user_id, trainset, item_map, top_n=config.N_RECOMMENDATIONS
):
    """
    ...
    user_interacted_items_inner = {
        item_inner_id for (item_inner_id, _) in trainset.ur[user_inner_id]
    }

    # Prepare list of items to predict (those the user hasn't interacted with)
    items_to_predict_inner = [
        iid for iid in all_item_inner_ids if iid not in user_interacted_items_inner
    ]
    ...
    testset_tuples = [
        (user_id, trainset.to_raw_iid(item_inner_id), 1.0)
        for item_inner_id in items_to_predict_inner
    ]
    ...
    if df_inter is not None:
        ...
        if train_df is not None and not train_df.empty:
            ...
            try:
                ...
                _, _, svd_item_map, _ = create_user_item_matrix(
                    train_df.copy()
                )  # Get item map specific to train set
                ...
                if not svd_item_map.empty:  # Check if item map was created
                    ...
                    recommendations_svd = get_cf_recommendations_surprise(
                        svd_model, example_user_svd, svd_trainset, svd_item_map
                    )
