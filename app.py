    ...
    if results:
        ...
        if "train_df" in train_data_components:
            available_users = sorted(
                train_data_components["train_df"]["user_id"].unique().tolist()
            )
...
def run_pipeline_for_flask():
    ...
    _, train_user_map, train_item_map, _ = main.preprocessing.create_user_item_matrix(
        train_df.copy()
    )
    ...
    if not train_item_metadata.empty:
        try:
            ...
            cb_similarity_matrix, cb_indices = (
                main.content_based.build_content_similarity_matrix(
    ...
    try:
        ...
        svd_algo, svd_trainset = main.collaborative_filtering.train_svd_model_implicit(
            train_df
        )
...
def index():
    ...
    return render_template(
        "index.html", users=available_users
...
def recommendations():
    ...
    if username and trained_models and train_data_components:
        ...
        if train_df is not None:
            ...
            print(
                f"Number of interactions for user '{username}' in training data: {len(user_interactions)}"
            )
        ...
        if svd_model_components and (
            not hasattr(train_item_map, "empty") or not train_item_map.empty
        ):
            try:
        ...
        train_df = train_data_components.get(
            "train_df"
        )  # Getting train_df again - this is redundant but okay for now
        if cb_model_components and not train_df.empty:
            try:
                user_liked_items = (
                    train_df[train_df["user_id"] == username]["item_id"]
                    .unique()
                    .tolist()
                )
                ...
                if user_liked_items:
                    cb_recs = main.content_based.get_content_based_recommendations(
                        ...
                        cb_model_components[2],  # sim_matrix, indices, all_items
                    )
                    if cb_recs:
                ...
                cb_recommendations = [
                    f"Error generating content-based recommendations: {e}"
                ]
        else:
